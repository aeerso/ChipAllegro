...
Chip8::Chip8()
{
    cout << "[ChipAllegro] Starting..\n";
}

Chip8::~Chip8()
{
    cout << "[ChipAllegro] Shutting down..\n";
}
...
void Chip8::init()
{
    //Allocating opcodes and memory space
    pc = 0x200; //first program counter to 0x200
    opcode = 0; //Reset opcodes
    I = 0;      //Reset index
    sp = 0;     //Reset stack pointer

    //TODO: clear the display via allegro
    ...
    cout << "[ChipAllegro] Stack, keypad and V register cleared.\n";

    for (unsigned i = 0; i < 4096; ++i)
    {
        memory[i] = 0;
    }
    cout << "[ChipAllegro] 4K memory cleared.\n";

    for (unsigned i = 0; i < 80; ++i)
    {
        memory[i] = chip8_fontset[i];
    }
    cout << "[ChipAllegro] Font loaded into memory.\n";

    delay_timer = 0;
    sound_timer = 0;

    srand(time(NULL));
    cout << "[ChipAllegro] Random seed generated.\n";
}
...
bool Chip8::load(const char *file_path)
{
    ...
    if (rom == NULL)
    {
        cout << "[ChipAllegro] Unable to load the ROM file.\n";
        return false;
    }

    //Get the ROM size (aka seeking until end, then rewind)
    fseek(rom, 0, SEEK_END);    //get the pointer to the end of file
    long rom_size = ftell(rom); //get the pointer
    rewind(rom);                //set back the pointer to 0

    //Memory allocate
    char *rom_buffer = (char *)malloc(sizeof(char) * rom_size);
    if (rom_buffer == NULL)
    {
        cout << "[ChipAllegro] Unable to allocate memory for ROM\n";
    }
    //Result is in an unsigned 16bit integer
    //Reading rom chunks of sizeof(char) saving them in rom_buffer
    size_t result = fread(rom_buffer, sizeof(char), (size_t)rom_size, rom);
    if (result != rom_size)
    {
        cout << "[ChipAllegro] Error: buffer mistmatch.\n";
        return false;
    }
    ...
    if ((4096 - 512) > rom_size)
    {
        ...
        {
            memory[i + 512] = (uint8_t)rom_buffer[i]; //Load into memory after loading fonts.
        }
    }
    else
    {
        cout << "[ChipAllegro] ROM too large for memory.\n";
        return false;
    }
